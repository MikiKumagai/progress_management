...
# 2. CSVに書き出す
with open("db/tasks.csv", mode='w', newline='', encoding='utf-8') as file:
    writer = csv.writer(file)
    writer.writerow(['name','progress_unit_id','progress_type_id','total_count','progress','active','is_wordbook'])
    for row in task_data:
        writer.writerow(row)

with open("db/progresses.csv", mode='w', newline='', encoding='utf-8') as file:
    writer = csv.writer(file)
    writer.writerow(['task_id','progress_value','progress_date'])
    for row in progress_data:
        writer.writerow(row)

with open("db/wordbook_entries.csv", mode='w', newline='', encoding='utf-8') as file:
    writer = csv.writer(file)
