...
from services import wordbook_service, progress_service, progress_unit_service, progress_type_service

class WordbookSetupPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        # ページタイトル
        title_label = ttk.Label(self, text="単語帳登録", font=("Helvetica", 16))
        title_label.grid(row=0, column=0, columnspan=6, padx=5, pady=10, sticky="nsew")

        # 単語帳名
        name_label = ttk.Label(self, text="単語帳名")
    ...
    def on_submit(self):
        name = self.name.get()
        if len(name) >= 10:
            self.name_error_label.config(text="単語帳名は10文字未満で入力してください")
            return
        else:
            self.name_error_label.config(text="")  # エラー解除
            return
        wordbook_service.add_wordbook(name)
